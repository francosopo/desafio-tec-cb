...
export class TransactionDone extends AbstractPaymentDatabaseStatus implements PaymentStatusInterface
{
    constructor() {
        super("Done");
    }
    ...
    process(status: PaymentInterface, token: string): Promise<PaymentResponseInterface> {
        ...
        status.getContext().setState(new SuccessfulResponseState(
